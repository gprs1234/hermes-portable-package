...
Converts a raw natural-language idea into a structured Concept Cell.
Reads from stdin or --text argument, outputs YAML-formatted concept cell.

Usage:
  echo "我想做攤車" | python3 concept_intake.py
  python3 concept_intake.py --text "status page is confusing"
  python3 concept_intake.py --text "build a small inventory app" --source user
...
def build_concept_cell(raw_idea: str, source: str = "user") -> dict:
...
def main():
    ...
    parser.add_argument("--source", default="user", help="Source (default: user)")
